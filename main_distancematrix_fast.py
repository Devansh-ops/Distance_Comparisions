...
class DistanceComparator:
    def __init__(self):
        ...
        self.distancematrix_api_key = os.getenv('DISTANCEMATRIX_API')
    ...
    def get_distance_distancematrix(self, start_long, start_lat, end_long, end_lat):
        url = f'https://api-v2.distancematrix.ai/maps/api/distancematrix/json?origins={start_lat},{start_long}&destinations={end_lat},{end_long}&key={self.distancematrix_api_key}'

        response = requests.get(url)
        if response.status_code == 200:
            data = response.json()
            if 'rows' in data and len(data['rows']) > 0 and 'elements' in data['rows'][0] and len(data['rows'][0]['elements']) > 0 and 'distance' in data['rows'][0]['elements'][0]:
                distance_meters = data['rows'][0]['elements'][0]['distance']['value']
                return distance_meters
            else:
                print("error in distancematrix - no property found")
                print(response.json())
                return None  # No feature or properties found in the response
        else:
            print("error in distancematrix")
            print(response.json())
            return None  # API request failed

    def compare_distances(self, google_distance, distancematrix_distance):
        if google_distance is None or distancematrix_distance is None:
            return "Error: One or both distances are unavailable."

        # Calculate absolute error
        absolute_error = abs(google_distance - distancematrix_distance)
        ...
        # Calculate accuracy
        accuracy = (min(google_distance, distancematrix_distance) / max(google_distance, distancematrix_distance)) * 100

        # Prepare and return the comparison metrics
        comparison_metrics = {
            "Google Distance (meters)": google_distance,
            "Distancematrix Distance (meters)": distancematrix_distance,
    ...
    def process_and_average_metrics(self, csv_file_path, limit=100):
        ...
        for start_long, start_lat, end_long, end_lat in self.read_random_rows(csv_file_path, limit):
            try:
                ...
                # start_time = time.time()
                distancematrix_distance = self.get_distance_distancematrix(start_long, start_lat, end_long, end_lat)
                # end_time = time.time()
                # print(f"Distancematrix API Request Time: {end_time - start_time} seconds")
                # print()
                metrics = self.compare_distances(google_distance, distancematrix_distance)
                # print("metrics", metrics)
                # print()
                if isinstance(metrics, dict):
                    request_counter += 1
                    sum_google_distance += metrics["Google Distance (meters)"]
                    sum_distancematrix_distance += metrics["Distancematrix Distance (meters)"]
    ...
    def print_average_metrics(self, total_entries, sum_google_distance, sum_distancematrix_distance, sum_absolute_error, sum_relative_error, sum_accuracy):
        avg_google_distance = sum_google_distance / total_entries
        avg_distancematrix_distance = sum_distancematrix_distance / total_entries
        ...
        print(f"Average Distancematrix Distance: {avg_distancematrix_distance} meters")
